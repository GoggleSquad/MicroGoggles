...
//calcula a regressão linear. fonte = https://www.statisticshowto.com/probability-and-statistics/regression-analysis/find-a-linear-regression-equation/
...
double correlation_datay_datax(int32_t *data_red, int32_t *data_ir)
{
	...
	double sy = 0;  //Desviãoo padrão de Y
...
#if !DEBUG
				return((int)resultado); //Não retorna daqui se estiver debugando.
#endif
			}
		}
	}
