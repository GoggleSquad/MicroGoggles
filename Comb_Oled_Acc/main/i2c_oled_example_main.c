...
// Accelerometer task function definition
void accel_display_task(void *pvParameters)
{
    vTaskDelay(pdMS_TO_TICKS(100));  // Wait for I2C initialization

    // Debug: Scan I2C bus for devices
    ...
    if (lis3dh_init(I2C_NUM_0) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize LIS3DH");
        vTaskDelete(NULL);
    }

    uint8_t whoami = lis3dh_who_am_i(I2C_NUM_0);
    if (whoami != 0x33) {
        ESP_LOGE(TAG, "LIS3DH not detected, WHO_AM_I: 0x%02X", whoami);
        vTaskDelete(NULL);
    }
    ...
    while (1) {
        if (lis3dh_read_accel(I2C_NUM_0, &x, &y, &z) == ESP_OK) {
            lis3dh_convert_to_g(x, y, z, &gx, &gy, &gz);
            snprintf(buf, sizeof(buf), "X: %.2f m/s²\nY: %.2f m/s²\nZ: %.2f m/s²", gx, gy, gz);

            _lock_acquire(&lvgl_api_lock);
            lv_label_set_text_static(accel_label, buf);
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(i2c_param_config(I2C_BUS_PORT, &i2c_config));
    ESP_ERROR_CHECK(i2c_driver_install(I2C_BUS_PORT, i2c_config.mode, 0, 0, 0));
...
//new addition 
    ...
    //new
    xTaskCreate(accel_display_task, "accel_display_task", 4096, NULL, 2, NULL);
